...
def decode(path: Path, ffmpeg: Path) -> tuple[float, str]:
    result = subprocess.run(
        [str(ffmpeg), "-v", "warning", "-i", str(path), "-ac", "1", "-ar", "16000", "-f", "s16le", "-"],
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("results/webm_decode_audit.tsv"))
    ...
    for path in selected:
        ...
        audit.append({
            "path": str(path), "container_seconds": f"{container:.6f}",
            "decoded_seconds": f"{decoded:.6f}", "delta_seconds": f"{decoded-container:.6f}",
            "warning": warning.replace("\t", " ").replace("\n", " | "),
        })
    args.output.parent.mkdir(parents=True, exist_ok=True)
    with args.output.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=audit[0], delimiter="\t", lineterminator="\n")
        writer.writeheader()
    ...
    print(json.dumps({
        ...
        "mean_absolute_duration_delta_seconds": sum(absolute_deltas) / len(absolute_deltas),
